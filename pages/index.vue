...
function editTask(task: TaskDTO) { editingTask.value = { ...task } }
async function deleteTask(task: TaskDTO) { if (!task.id) return await store.remove(task.id) }
function onSave(taskData: { title: string; description?: string; status?: TaskStatus }) {
  if (editingTask.value) { store.update(editingTask.value.id, taskData); editingTask.value = undefined }
  else { store.add(taskData) }
}
...
<template>
  ...
  <div class="mb-4 flex gap-2">
    <button @click="currentTab = 'list'" :class="currentTab==='list'? 'bg-blue-500 text-white':'bg-gray-200' " class="px-4 py-2 rounded">List</button>
    <button @click="currentTab = 'kanban'" :class="currentTab==='kanban'? 'bg-blue-500 text-white':'bg-gray-200' " class="px-4 py-2 rounded">Kanban</button>
  </div>
  ...
  <div v-else>
    <KanbanBoard :tasks="store.tasks" @edit="editTask" @delete="deleteTask"/>
  </div>
</template>
